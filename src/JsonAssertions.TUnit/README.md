...
| `IsEquivalentJsonTo(expected)` / `IsEquivalentJsonTo(expected, configure)` | Asserts the whole document is structurally equivalent to `expected`, independent of property order and number form (`1` == `1.0`). The configure callback sets `IgnorePath` and `IgnoreArrayOrder`. |
...
**Requirements:** TUnit 1.54.0 or later, .NET 10. `System.Text.Json` is in-box on .NET 10, so the package carries no runtime dependency beyond TUnit.

# Contributing to JsonAssertions.TUnit

Thanks for considering a contribution. This document covers the small set of conventions that keep the project consistent.

## Reporting bugs

Use the [bug report template](https://github.com/JohnVerheij/JsonAssertions.TUnit/issues/new?template=bug_report.yml). The template asks for the JsonAssertions.TUnit version, TUnit version, .NET SDK version, OS, expected vs actual behavior, and a minimal reproduction. The smaller and more self-contained the repro, the faster the fix.

## Proposing features

Use the [feature request template](https://github.com/JohnVerheij/JsonAssertions.TUnit/issues/new?template=feature_request.yml). Describe the test scenario the feature enables, the proposed API shape (in code), and any alternatives you considered. Indicate whether you'd like to submit a PR yourself.

For larger ideas (new entry points, breaking changes, cross-cutting refactors), open a [Discussion](https://github.com/JohnVerheij/JsonAssertions.TUnit/discussions) first to align on direction before investing implementation time.
...
3. Add or update tests. The project uses TUnit; existing tests in `tests/JsonAssertions.TUnit.Tests/` show the patterns.
...
- **Argument validation:** public methods that take reference-type arguments use `ArgumentNullException.ThrowIfNull(...)` (or `ArgumentException.ThrowIfNullOrWhiteSpace(...)` for strings) at the top of the body.
...
- Tests live in `tests/JsonAssertions.TUnit.Tests/` (main behavior, drives the coverage gate) and `tests/JsonAssertions.TUnit.SnapshotTests/` (public API surface pin via `MatchesSnapshot()`).
- Each public method should have at least one test covering its happy path and at least one covering an invalid-input or failing path.
...
Releases are published to NuGet via a tagged commit on `main`. The `<Version>` in `JsonAssertions.TUnit.csproj` is the source of truth for the package version.

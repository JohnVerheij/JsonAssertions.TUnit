...
JsonAssertions.TUnit is a personal-name open-source project distributed under the [MIT license](LICENSE). Although the package is intended for use **only in test projects** (see the scope blockquote on every README), security reports are still welcome and treated as a priority.
...
1. Navigate to the repository's [Security tab](https://github.com/JohnVerheij/JsonAssertions.TUnit/security)
...
|---|---|---|
| `0.0.x` | **Current** (skeleton preview) | ✅ Yes |
| `< 0.0.1` | Pre-release | ❌ No |
...
- The shipped `JsonAssertions.TUnit` NuGet package
- The build pipeline (release workflows, signing, NuGet publishing chain) and supply-chain integrity of the package
...
- Realistic in-product attack surface examples:
  - Unbounded memory or CPU consumption from a crafted assertion chain or a crafted JSON input
...
- Vulnerabilities in **the .NET runtime / SDK** itself, including `System.Text.Json`. Report to [Microsoft Security Response Center](https://msrc.microsoft.com/).
...
Replace `<version>` (e.g. `0.0.1`) with the artifact you downloaded:

```bash
# Verify SLSA v1.0 build provenance: was this nupkg built from this audited source tree?
gh attestation verify ./JsonAssertions.TUnit.<version>.nupkg --repo JohnVerheij/JsonAssertions.TUnit

# Verify the CycloneDX SBOM: is this BOM the authentic SBOM for that nupkg?
gh attestation verify ./JsonAssertions.TUnit.<version>.nupkg \
    --repo JohnVerheij/JsonAssertions.TUnit --predicate-type https://cyclonedx.org/bom
```
...
| Build to nupkg artifact | `actions/attest-build-provenance@v4` | [SLSA v1.0 Provenance](https://slsa.dev/spec/v1.0/provenance) |
